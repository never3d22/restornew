...
1. **Создайте `.env`:**
   ...
   ```env
   VITE_API_URL=https://roteelonoogu.beget.app/trpc
   ```
   Для локальной разработки можно указать `http://127.0.0.1:3000/trpc`.
2. **Установите зависимости:** `npm install`.
3. **Локальный запуск:** `npm run dev -- --host 0.0.0.0 --port 5173` и откройте `http://<IP>:5173` в браузере. Авторизуйтесь через виджет в шапке, админ-секрет вводится вручную.
4. **Сборка production-версии:** `npm run build`. Готовый статический бандл появится в `web/dist/`.
5. **Предпросмотр собранной версии:** `npm run preview -- --host 0.0.0.0 --port 4173` (удобно проксировать через Nginx на отдельный поддомен, см. [docs/web-client.md](docs/web-client.md)).

> ⚠️ В веб-клиенте админские действия доступны только после ручного ввода секрета — он не хранится в `.env`, чтобы исключить утечки.
